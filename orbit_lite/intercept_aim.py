...
def intercept_angle(
    ...
    *,
    ...
    # contact == -1 and eta_c < H usually implies an environment death (Sun or OOB).
    viable = (contact == tgt)
    ...
    if sun_blocked.any():
        ...
        # Vectorize verification over [M_blocked, O_offsets]
        M_sb = sb_idx.shape[0]
        O = offsets.shape[0]
        ...
        # Flatten to score all offsets at once: [M_sb * O]
        ...
        # Reshape back to [M_sb, O] and find the first success per candidate
        success = (c_m == tgt[sb_idx].repeat_interleave(O)).reshape(M_sb, O)
        if success.any():
            # For each candidate that succeeded, pick the smallest magnitude offset (or just the first success)
            # The offsets list is already ordered by priority.
            has_success = success.any(dim=1)
            first_success_off = success.to(torch.int8).argmax(dim=1) # [M_sb]

            s_cand_idx = sb_idx[has_success]
            s_off_idx = first_success_off[has_success]
            # Advanced index the winners from the [M_sb, O] grids
